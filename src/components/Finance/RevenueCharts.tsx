...
import {
  ...
  lookupCommissionPct, getPreviousMonthKey, formatEur,
} from '../../utils/finance';
...
function getTourLiquido(tour: Tour, feesArray: MonthlyFees[], commissionArray: MonthlyPrivateCommission[], repeatsPerMonth: Record<string, number>): number {
  ...
  return calcTourFinance(tour, fees, commissionPct).liquido;
}

export default function RevenueCharts({ tours, feesArray, commissionArray, repeatsPerMonth }: Props) {
  const today = new Date();

  const monthKeys = Array.from({ length: 12 }, (_, i) => format(subMonths(today, 11 - i), 'yyyy-MM'));
  const monthLabels = monthKeys.map(k => {
    const [y, m] = k.split('-').map(Number);
    return format(new Date(y, m - 1, 1), 'MMM', { locale: pt });
  });

  const completed = tours.filter(t => t.revenueTotal > 0);

  const monthNetMap: Record<string, number> = {};
  for (const t of completed) {
    const mk = t.date.slice(0, 7);
    const net = getTourLiquido(t, feesArray, commissionArray, repeatsPerMonth);
  ...
  const typeData = typeKeys.map(type => {
    const net = completed
      ...
      .reduce((s, t) => s + getTourLiquido(t, feesArray, commissionArray, repeatsPerMonth), 0);
    ...
    <div className="mb-6 md:mb-8 flex flex-col gap-4">
      {hasMonthData && (
        <div className="bg-white dark:bg-[#111] border border-black/[0.08] dark:border-white/[0.08] rounded-2xl p-4 md:p-5 shadow-card">
          <div className="text-[10px] font-semibold uppercase tracking-widest text-[#6b6b6b] dark:text-[#888] mb-4">Receita líquida — últimos 12 meses</div>
          <svg viewBox={`0 0 ${CHART_W} ${SVG_H}`} width="100%" height="auto" aria-hidden="true" style={{ display: 'block' }}>
            {barValues.map((v, i) => {
              ...
              return (
                <g key={monthKeys[i]}>
                  <rect x={x} y={y} width={barW} height={barH} rx={4} fill="#002FA7" fillOpacity={0.75}>
                    <title>{monthLabels[i]}: {formatEur(v)}</title>
                  </rect>
                  {barH > 22 && (
                    <text x={labelX} y={y - 4} textAnchor="middle" fontSize={9} fill="#6b6b6b">{formatEur(v)}</text>
                  )}
                  <text x={labelX} y={CHART_H + 18} textAnchor="middle" fontSize={10} fill="#6b6b6b">{monthLabels[i]}</text>
                </g>
              );
            })}
          </svg>
      ...
      {hasTypeData && (
        <div className="bg-white dark:bg-[#111] border border-black/[0.08] dark:border-white/[0.08] rounded-2xl p-4 md:p-5 shadow-card">
          <div className="text-[10px] font-semibold uppercase tracking-widest text-[#6b6b6b] dark:text-[#888] mb-4">Receita líquida por tipo de tour</div>
            ...
            {typeData.map(({ type, net, label, color }) => (
              <div key={type} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-5 bg-black/[0.05] dark:bg-white/[0.06] rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-500" style={{ width: net > 0 ? `${(net / maxTypeNet) * 100}%` : '0%', backgroundColor: color, opacity: 0.8 }} />
                </div>
                <span className="w-20 shrink-0 text-xs font-semibold text-green-500 text-right">{net > 0 ? formatEur(net) : '—'}</span>
              </div>
            ))}
          </div>
        </div>
