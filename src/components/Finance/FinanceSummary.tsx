...
import type { TourTotals } from '../../utils/finance';
...
function SummaryCard({ label, data }: { label: string; data: TourTotals }) {
  return (
    <div className="bg-white dark:bg-[#111] border border-black/[0.08] dark:border-white/[0.08] rounded-2xl p-4 md:p-5 shadow-card">
      ...
      <div className="grid grid-cols-2 gap-2 md:gap-3">
        <div>
          ...
          <div className="text-xl md:text-2xl font-semibold text-ink dark:text-[#e8e5e0]">{data.count}</div>
        </div>
        <div>
          <div className="text-[10px] text-[#6b6b6b] dark:text-[#888] mb-0.5">Pax</div>
          <div className="text-xl md:text-2xl font-semibold text-ink dark:text-[#e8e5e0]">{data.pax}</div>
        </div>
        <div>
        ...
        <div>
          <div className="text-[10px] text-[#6b6b6b] dark:text-[#888] mb-0.5">IVA + Taxas</div>
          <div className="text-sm font-medium text-red-500">{formatEur(data.iva + data.feeTotal)}</div>
        </div>
        <div className="col-span-2 pt-2 border-t border-black/[0.06] dark:border-white/[0.06]">
          ...
          <div className="text-2xl md:text-3xl font-semibold text-green-500">{formatEur(data.liquido)}</div>
        </div>
      </div>
    </div>
  );
...
export default function FinanceSummary({ tours, feesArray, commissionArray, repeatsPerMonth }: Props) {
  ...
  const withData = tours.filter(t => t.revenueTotal > 0);
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 mb-6 md:mb-8">
      <SummaryCard label="Este mês" data={calcTotals(thisMonth, feesArray, commissionArray, repeatsPerMonth)} />
      <SummaryCard label="Esta semana" data={calcTotals(thisWeek, feesArray, commissionArray, repeatsPerMonth)} />
      <SummaryCard label="Hoje" data={calcTotals(thisDay, feesArray, commissionArray, repeatsPerMonth)} />
    </div>
  );
}
